...
let umweltereignis = 0;
let ereignis = "Keins" // nimmt Wert des Ereignisses an für die Tabelle
...
function SchmetterlingeAnzahl() {
    if (Ackerspritzung === true) {
        Schmetterlinge = Math.floor(Schmetterlinge*0.5);
    ...
    if (Starkregenereignis === true && Starkregencounter === 1) {
        Schmetterlinge = Math.floor(Schmetterlinge*0.2);
...
function Ereignismaster(){
    ...
    if (umweltereignis == 0){
        ...
        Ereignisse() 
            ...
            if (periodencounter % umweltereignis === 0 && periodencounter != 0) {
                Ereignisse();

            }
            else {
                if (umweltereignis <= 5) {
                    ereigniscounter = Math.floor(Math.random() * 4) + 1;
                    ...
                    ereigniscounter = Math.floor(Math.random() * 6) + 1;
                    if (ereigniscounter == 1) {
                        Ereignisse();
                    }
                    else {
...
function reset() {
    ...
    counter = 0;
    Ereignis = "Keins";
    localStorage.setItem('ereignis' + periodencounter, "Keins");
    localStorage.setItem('eier' + periodencounter, 0);
    localStorage.setItem('raupen' + periodencounter, 0);
